...
namespace SRLfem
{
    ...
    void MatSolversICCG::sortAlgebraicMultiColor(slv_int size0, const SparseMatBaseD &A, SparseMatBaseD &PA, const double *b, double *Pb, std::vector<std::vector<int>> &color_list, std::vector<int> &ordering, std::vector<int> &reverse_ordering, int num_colors)
    {
        ...
        int icolor = 0;
        ordering.resize(num_row);
        reverse_ordering.resize(num_row);
        color_list.resize(num_colors_);
        std::vector<slv_int> color(num_row);
        for (int i = 0; i < num_row; i++)
        {
            color[i] = -1;
            int mu = start_pos[i];
    ...
    {
        const slv_int num_row = size0;
        int max_block = (num_row + num_blocks - 1) / num_blocks;
        ...
        std::vector<slv_int> block_assign(num_row, -1);
        for (int i = 0; i < max_block; i++)
        {
            for (slv_int j = 0; j < num_row; j++)
            {
                if (block_assign[j] == -1)
                {
                    block_assign[j] = i;
                                                      ...
                                                      int num_colors)
    {
        ...
        makeAlgebraicBlock(num_row, A, Mb, reverse_block_list, num_blocks);
                                          ...
                                          int numItr)
    {
        ...
        //     block_list......list of algebraic blocks
        //     color_list......list of colors
        //     diagD......diagonal elements of A
        ...
        //
        //    ==== output ====
        //    x[i]...residual vector (i=0,1,2,...n-1)
        //
        //    ==== return value ====
...
#pragma omp parallel for
        ...
        parallelIcSolv(size0, col_ptr, val_ptr, start_pos, end_pos, col_ptr2, val_ptr2, start_pos2, end_pos2, block_list, color_list, diagD, r, ru);
...
#pragma omp parallel for
    ...
    /* *********************************************************** */
    void MatSolversICCG::parallelIcSolv(slv_int size0, const int *col_ptr, const double *val_ptr, slv_int *start_pos, slv_int *end_pos, const int *col_ptr2, const double *val_ptr2, slv_int *start_pos2, slv_int *end_pos2, std::vector<std::vector<slv_int>> &block_list, std::vector<std::vector<slv_int>> &color_list,
...
#pragma omp parallel for private(bl, l, mu, t, idx)
            for (k = 0; k < color_list[col].size(); k++)
            {
                ...
                for (l = 0; l < block_list[bl].size(); l++)
                {
                    ...
                    {
                        t -= val_ptr[mu] * x(col_ptr[mu]);
                    }
                    x(idx) = t / val_ptr[end_pos[idx] - 1]; // SparseMat IC version
                }
            }
        }
...
#pragma omp parallel for private(bl, l, nu, t, idx)
    ...
    bool MatSolversICCG::solveICCGwithABMC(const slv_int size0, const double conv_cri, const int max_ite, const double accera, const double normB, const SparseMat &matA, const double *vec_b, double *vec_x, int num_blocks, int num_colors, bool init)
    {
        ...
        // Solve main
        is_conv = MatSolversICCG::parallelIccgSolv(n, *(PA.matrix), *(matL.matrix), *(matL_tr.matrix), block_list, color_list, diagD, vec_Pb, vec_r, epsilon, max_ite);
